...
{   
    ...
    if (z>800 && ifsprintOnce==0 )   // 在找球，还没冲刺
    {   
        // twist.linear.x = -0.58;
    ...
    else if ( z<800 && z!=-1 && ifsprintOnce==0)  // 正在冲刺
    {   
        ROS_INFO("sprint");
...
int linemap[] = {1,3, 8, 7,7,7,7,7,7,7} ;
int readlinemap() //line condition mode
{
    int linemapIndex;
...
//int myMap[] = {1, 0, 255, 3, 0, 255, 255, 255, 255, 255};
int myMap[] = {1, 101, 0,105,97, 10,96, 254, 11, 12, 13, 254, 253, 14,  253, 15, 16, 17, 21, 14, 254, 255, 255, 255};
...
{   
    ...
    while (ros::ok())
    {
        ...
        {   case -1:
            ...
            case 31:
                {
                    ROS_INFO("Case 31: moving odomAlignlineFollow to boxes");
                    ROS_INFO("targetOdom[0]: %f", targetOdom[0]);
                    ROS_INFO("targetOdom[1]: %f", targetOdom[1]);
                    odomAlignlineFollow();
                    // if(abs(errorOdom[0]) < 0.6 && ending_three == 0)
