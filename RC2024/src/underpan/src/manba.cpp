...
void 2BallarrayCallback(const std_msgs::Float64MultiArray::ConstPtr& array)
{
    geometry_msgs::Twist twist;
    // std_msgs::Int8 msg;
    // msg.data = 2;
    ...
    double last_erro, erro, d_erro;
    ROS_INFO("z: %f, y: %f, color: %f", z, y, c);
    if(z = -1 && ifsprintOnce == 0)
    {
        twist.linear.x = 0;
        twist.linear.y = 0.49;
        twist.angular.z = -float(errorOdom[2])*0.001;
        if(abs(twist.angular.z)<0.12){
            twist.angular.z = (errorOdom[2] > 0) ? 0.15 : -0.15;
        }
        underpan_control.publish(twist);
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    else if(c == 3 && ifsprintOnce == 0){
        if(abs(y)>10){
            twist.linear.y = -float(y)*0.001;
            twist.linear.y = (y>0) ? -0.28:0.28;
            twist.linear.x = 0;
            underpan_control.publish(twist);
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }
...
int linemap[] = {1,1 ,3,3,8,7,7, 7, 7,7,7,7,7,7,7};
int readlinemap() //line condition mode
{
    int linemapIndex;
    ...
    else if (crossmode != 0 && islinecatchable==1)
    {
        ...
        switch (line_condition)
        {
            ...
            case 8:
                {
                    turnmode = "crossover";
                    twist.angular.z = 0.986;
                    twist.linear.x = 0;
                    twist.linear.y = 0;
                    underpan_control.publsh(twist);
                    std::this_thread::sleep_for(std::chrono::milliseconds(300));
                    twist.linear.x = -0.55;
                    twist.linear.y = 0;
                    twist.angular.z = 0;
...
void writeOdom(double rel_x = 0.0, double rel_y = 0.0, double rel_z = 0.0)
{
    if(writeOdomflag ==1)
    {
        targetOdom[0] = nowOdom[0] + rel_x - now[0];
        targetOdom[1] = nowOdom[1] + rel_y - now[1];
        targetOdom[2] = nowOdom[2] + Rotate2Range(rel_z) - now[2];
        writeOdomflag = 0;
    }
    if(writeOdomflag ==2)
    {
        waitToOdom[0] = nowOdom[0] + rel_x - now[0];
        waitToOdom[1] = nowOdom[1] + rel_y - now[1];
        waitToOdom[2] = nowOdom[2] + Rotate2Range(rel_z) - now[2];
        writeOdomflag = 0;
    }
}
...
//int myMap[] = {1,101,1, 21,105, 0,105,10, 254, 11, 12, 13, 254, 253, 14,  253, 15, 16, 17, 21, 14, 254, 255, 255, 255}; //测试放球
// int myMap[] = {1, 2, 255, 3, 0, 255, 255, 255, 255, 255};

/*调试用*/
int myMap[] = {1, 96, 253, 1, 3, 4, 101, 0, 105, 10, 254, 11, 12, 13, 254, 253, 14, 253, 15, 16, 17, 21, 14, 254, 255, 255, 255};//开始转弯
int myMap[] = {1, 1, 97, 3, 4, 101, 0, 105, 10, 254, 11, 12, 13, 254, 253, 14, 253, 15, 16, 17, 21, 14, 254, 255, 255, 255};//开始no转弯
int myMap[] = {1, 1, 97, 3, 4, 81, 253, 97, 101, 80, 105, 82, 253, 97, 10, 254, 11, 12, 13, 254, 253, 14, 253, 15, 16, 17, 21, 14, 254, 255, 255, 255};//开始no转弯,xun qiu gai
...
{   
    ...
    ODOM_control = nh.advertise<std_msgs::Int8> ("cmd_odom0", 5);
            ...
            case 101:
                {
                    ...
                    writeOdom(0, 0, 0);
                    ROS_INFO("TAGET-ODOM X: %f, Y: %f, Z: %f", targetOdom[0], targetOdom[1], targetOdom[2]);
                    ROS_INFO("NOWODOM X: %f, Y: %f, Z: %f", nowOdom[0], nowOdom[1], nowOdom[2]);
                    condition = -1;
                    break;
                }
            case 102:
