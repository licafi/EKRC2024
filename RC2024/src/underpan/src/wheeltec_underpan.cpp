...
***************************************/
void turn_on_robot::Cmd_odomClearFlag_Sub(const std::msgs::Int8 odomflag)
{
  if(odomflag == 1)
  {
    odomClearFlag = 1;
...
功能: 构造函数, 只执行一次，用于初始化
***************************************/
void odomClear(bool odomClearFlag){
  if (odomClearFlag == 1){
    Robot.Pos.X = 0;
    Robot.Pos.Y = 0;
    Robot.Pos.Z = 0;
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  Cmd_odomClearFlag_Sub = = n.subscribe("cmd_odom", 1, &turn_on_robot::Cmd_Vel_Callback, this);
